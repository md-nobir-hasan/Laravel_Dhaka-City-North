...
//         }
    ...
    public function show_unit_report(request $request)
    {
       ...
       $u_id = $request->u_id;
        ...
        'u_id.required' => 'অনুগ্রহ করে ইউনিট কমটি বাছাই করুন'
        ]);

      
        // dd($request->ps_id,$request->w_id,$request->u_id);
       $data_p = parlament_seat::orderBy('no')->get();


        if($ps_id=='সকল' && $w_id=='সকল' && $u_id=='সকল')
        {
            ...
            ->join('words','unit_r_p_s.w_id','=','words.id')
            ->join('units','unit_r_p_s.u_id','=','units.id')
            ->select('unit_r_p_s.*','police_stations.*','designation.*','units.*','parlament_seat.*','words.*')
            ->where('unit_r_p_s.p_id','=',"$request->p_id")
            ->where('designation.d_name','=',"সভাপতি")
            ->orderBy('unit_r_p_s.u_id')
            ->get();
            ...
            $data_unit2 = DB::table('unit_r_p_s')
                        ...
                        return view('/display_unit_rp_info',compact('data_unit',"data_p","data_unit2","all_thana","all_ward","all_unit"));

            }
        elseif($ps_id!='সকল' && $w_id!='সকল' && $u_id=='সকল')
        {
        ...
        //    $data_than = DB:table()
           ...
           $data_unit = DB::table('unit_r_p_s')
            ...
            ->orderBy('unit_r_p_s.u_id')
            ->get();
            $all_thana =$data_unit[0]->PS_name." থানা";
            $all_ward =$data_unit[0]->w_number." ওয়ার্ড";
            $all_unit ="সকল ইউনিট";
            // dd($data_unit);
        ...
        //    $data_than = DB:table()
           ...
           $data_unit = DB::table('unit_r_p_s')
            ...
            $all_thana =$data_unit[0]->PS_name." থানা";
            $all_ward =$data_unit[0]->w_number." ওয়ার্ড";
            $all_unit =$data_unit[0]->union_name." সকল ইউনিট";
           ...
           $data_unit = DB::table('unit_r_p_s')
            ...
            //    $data_than = DB:table()
               ...
               $data_unit = DB::table('unit_r_p_s')
                ...
                ->where('unit_r_p_s.p_id','=',"$request->p_id")
                ->where('unit_r_p_s.w_id','=',"$request->w_id")
                ->where('unit_r_p_s.w_id','=',"$request->u_id")
                ->where('designation.d_name','=',"সাধারণ সম্পাদক")
...
return view('/display_mp',compact('data_p'));
    }
